...
export const padArrayInner = function (arr: string[], segment: number, len: number): string[] {
  let padded: string[] = [];
  let slice: string[];
...
export const hasSize = function (symbolId:number): boolean {
  const size = symbolSize(symbolId);
  return size != undefined;
}

export const getOffset = function (el:HTMLElement): {top:number, left:number} {
...
export const overlap = function (el1:HTMLElement, el2:HTMLElement): boolean {
  if (!el2) return false;
