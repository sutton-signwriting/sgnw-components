        ...
        <span class="outside"><span class="middle"><span class="inside">
          {this.items.map( (item) => {
            ...
            if (item['segment'] == 'sign'){
              ...
              return <sgnw-sign styling={styling} style={{"font-size": (values.zoom * 30) + "px", "width": width + "px", "height": height + "px", "margin-right": ((right>0)?right:0) + "px", "border-left": ((right<0)?(-right):0) +"px solid transparent", "padding": padding + "px"}}>{item['text']}</sgnw-sign>
              ...
              height = item['height'] * zoom + pad * 2;
              padding = Math.max(0,(20 * zoom) - pad);
              return <sgnw-symbol styling={styling} style={{"font-size": (values.zoom * 30) + "px", "width": width + "px", "height": height + "px", "padding-bottom": padding + "px"}}>{item['text']}</sgnw-symbol>
            } else {
              console.log("other")
