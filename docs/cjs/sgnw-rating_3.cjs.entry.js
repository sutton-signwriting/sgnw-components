...
/* Sutton SignWriting Core Module v1.2.0 (https://github.com/sutton-signwriting/core), author: Steve Slevinski (https://SteveSlevinski.me), license: MIT */
let o$3={colorize:"C",colorhex:"(?:[0-9a-fA-F]{3}){1,2}",colorname:"[a-zA-Z]+",padding:"P[0-9]{2}",zoom:"Z(?:[0-9]+(?:\\.[0-9]+)?|x)",zoomsym:"Z[0-9]{2},[0-9]+(?:\\.[0-9]+)?(?:,[0-9]{3}x[0-9]{3})?",classbase:"-?[_a-zA-Z][_a-zA-Z0-9-]{0,100}",id:"[a-zA-Z][_a-zA-Z0-9-]{0,100}"};o$3.colorbase=`(?:${o$3.colorhex}|${o$3.colorname})`,o$3.color=`_${o$3.colorbase}_`,o$3.colors=`_${o$3.colorbase}(?:,${o$3.colorbase})?_`,o$3.background=`G${o$3.color}`,o$3.detail=`D${o$3.colors}`,o$3.detailsym=`D[0-9]{2}${o$3.colors}`,o$3.classes=`${o$3.classbase}(?: ${o$3.classbase})*`,o$3.full=`-(${o$3.colorize})?(${o$3.padding})?(${o$3.background})?(${o$3.detail})?(${o$3.zoom})?(?:-((?:${o$3.detailsym})*)((?:${o$3.zoomsym})*))?(?:-(${o$3.classes})?!(?:(${o$3.id})!)?)?`;const t$2=e=>{if("object"!=typeof e||null===e)return;let s="-";s+=e.colorize?"C":"";const t=parseInt(e.padding);s+=!t||t<=0||t>99?"":"P"+(t>9?t:"0"+t);const a=e.background&&"string"==typeof e.background?e.background.match(o$3.colorbase)[0]:void 0;s+=a?"G_"+a+"_":"";const i=e.detail&&e.detail[0]&&"string"==typeof e.detail[0]?e.detail[0].match(o$3.colorbase)[0]:void 0,l=e.detail&&e.detail[1]&&"string"==typeof e.detail[1]?e.detail[1].match(o$3.colorbase)[0]:void 0;i&&(s+="D_"+i,l&&(s+=","+l),s+="_");const r="x"===e.zoom?"x":parseFloat(e.zoom);s+=!r||r<=0?"":"Z"+r;let c="";c+=(e.detailsym&&Array.isArray(e.detailsym)?e.detailsym.map(e=>{const s=parseInt(e.index);if(!s||s<=0||s>99)return "";let t="D"+(s>9?s:"0"+s);const a=e.detail&&e.detail[0]?e.detail[0].match(o$3.colorbase)[0]:void 0,i=e.detail&&e.detail[1]?e.detail[1].match(o$3.colorbase)[0]:void 0;return a&&(t+="_"+a,i&&(t+=","+i),t+="_"),t}):[]).join(""),c+=(e.zoomsym&&Array.isArray(e.zoomsym)?e.zoomsym.map(o=>{const e=parseInt(o.index);if(!e||e<=0||e>99)return "";let s="Z"+(e>9?e:"0"+e);const t=parseFloat(o.zoom);if(s+=!t||t<=0?"":","+t,o.offset&&0 in o.offset&&1 in o.offset){const e=parseInt(o.offset[0])+500,t=parseInt(o.offset[1])+500;e>=250&&e<750&&t>=250&&t<750&&(s+=","+e+"x"+t);}return s}):[]).join("");let d="";const n=e.classes&&"string"==typeof e.classes?e.classes.match(o$3.classes)[0]:void 0;d+=n||"";const m=e.id&&"string"==typeof e.id?e.id.match(o$3.id)[0]:void 0;return d+=n||m?"!":"",d+=m?m+"!":"",s+(c||d?"-"+c:"")+(d?"-"+d:"")};
...
const SgnwSymbol = class {
    ...
    render() {
        ...
        if (this.styling) {
            ...
            styleStr = t$2(styleObj);
        }
        //var svgSize = parseFloat(window.getComputedStyle(this.el).getPropertyValue("font-size").slice(0,-2))/30;
        return (global$1.h(global$1.Host, { iid: this.iid, fsw: this.fsw, swu: this.swu, styling: this.styling, innerHTML: this.sgnw ? w(this.fsw + (styleStr)) : '' }, global$1.h("slot", null)));
    }
